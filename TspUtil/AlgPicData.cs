...
namespace TspUtil
{
    ...
    public class DataBmpAlg
    {
        ...
        public bool IsCalculateChecksum { get => _isCalculateChecksum; set => _isCalculateChecksum = value; }
        ...
        public DataBmpAlg(Gbl gbl, byte[] by, MaskForArgbItem oddmask, MaskForArgbItem evenmask, PadLoc padLoc)
        {
            ...
            _isCalculateChecksum = _gbl.IsCalculateChecksum;
            ...
            using (var ms = new MemoryStream(by, false))
            {
                ...
                if (_gbl.IsNetWorkSend)
                {
                    ...
                    _img = new Bitmap(ms);
                    _srcData = DecodeData();
                    _exdata = DataMultiExtract(_srcData);
                   
                    List<byte> pac = DataPackaging();
                    if (IsPicReArrag)
                    {
                    ...
                    {
                        _checkSum = GetCheckSum(FinalData);//计算checkSum,校验和
                    }
        ...
        //
        /// <summary>
        /// 数据提取，将期望的数据提取到list里面去 1377ms左右
        /// </summary>
        ...
        public List<byte>[] DataExtract(byte[] src)
        {
           ...
           // var argbL
            Func<int, string, List<byte>> act = (row, expRgbA) =>
            {
                ...
                var argblst = ArgbLstCal(argl);     //计算每个像素点需要用的配置
                for (int widthIdx = 0; widthIdx < ImgWidth; widthIdx += gap)
                {
                    ...
                    Array.Copy(src, srcIndex + intenal, resrc, 0, resrc.Length);//copy时间不稳定，会时常有奇点1-2tic
                    var fourData = GetData(resrc, argblst);//对数据进行排序后输出 1-2tick
                  //  CheckSumColorModel(len, fourData,banLst, ref rgData, ref bgData);
                    if (IsCross)
                    {
            ...
            for (int row = 0; row < ImgHeight; row++)
            {
                ...
                //st.Stop();
                //var sp = st.ElapsedMilliseconds;
                //_log.Debug($"提取一行所用时间{st.ElapsedMilliseconds}ms,{st.ElapsedTicks}");
            }
            return extractArr;
        }

        private List<int> GetBanLst(int gap, int widthIdx, int row)
        {
            var banLst = new List<int>();
            if (!IsCalculateChecksum)//使用列表规则数据，只计算列表规则数据，
            {
                for (int i = 0; i < gap; i++)// 
                {
        ...
        public List<byte>[] DataMultiExtract(byte[] src)
        {
            var halfInperiorIndex = _gbl.InfriorHalf;
            var extractArr = new List<byte>[ImgHeight];
            var halfImgForCross = IsInferior ? _gbl.HalfImageIndex : (ImgWidth / 2) - 1;//图片一行中的一半的前一个数据，图片须为偶数列width为偶数
            Func<int, string,int, List<byte>> act = (row, expRgbA, width) =>
            {
                var tmplst = new List<byte>();
                var argl = ArgbItemHelper.PraseArgbText(expRgbA);//将从界面获取的argb数据转换成数组
                var cap = (argl.Max(p => p.Idx) + 1) * _bpp;   //计算界面中获取的像素index，计算数据个数
                ...
                var lst = new List<byte>();
                var gap = cap / _bpp;               //统计像素个数
                var allLineByte = _bpp * width;  //每行的数据个数
                var intenal = row * allLineByte;    //行开头第一个的数据索引
                var argblst = ArgbLstCal(argl);     //计算每个像素点需要用的配置
                for (int widthIdx = 0; widthIdx < width; widthIdx += gap)
                {
                    var srcIndex = widthIdx * _bpp; //当前的像素索引
                    var len = (srcIndex + cap) < allLineByte ? cap : (allLineByte - srcIndex);//判断当前像苏数据是否超过本行最后一个数据
                                                                                              // var banLst = GetBanLst(gap, widthIdx, row);
            ...
            var oddFirstLines = new List<List<byte>>();
            var evenFirstLines = new List<List<byte>>();
            var imgH = ImgHeight;
            for (int row = 0; row < imgH; row += 2)
            {
                ...
                {
                    oddFirstLines.Add(act(row, OddMask.ExpRGBA, width));

                });

                var tsk2 = Task.Factory.StartNew(() =>
                {
                    evenFirstLines.Add(act(row + 1, EvenMask.ExpRGBA, width)); 
            ...
            Task.Factory.StartNew(() =>
            {
                ...
                for (int row = 0; row < imgH; row += 2)
                {
                    extractArr[row] = oddFirstLines[index];
                    extractArr[row + 1] = evenFirstLines[index];
                    index++;
                }
            });
        ...
        /// <returns>O(N)</returns>
        public byte[] GetData(byte[] src, List<int> argblst)
        {
            var tempList = new byte[argblst.Count];
            var index = 0;
            foreach (var n in argblst)
            {
                tempList[index++] = src[n];
            }
        ...
        /// <param name="argblst"></param>
        /// <returns></returns>
        public List<int> ArgbLstCal(List<ArgbItem> argblst)
        {
            var banLst = new List<int>();
        ...
        public List<byte> GetCheckSumData(byte[] capData, List<Argb> argblst, int n, int[] allow)
        {
        ...
        public int CheckSumColorModel(int len, byte[] src, int[] allowLst, ref List<byte> rgData, ref List<byte> bgData)
        {
            int n = len / _bpp;
        ...
        public byte[] GetCheckSum(List<byte> sourceData)
        {
            uint crc = 0xffffffff;
            byte[] CheckSum = new byte[8];
            ...
            {
                crc = crc + (uint)sourceData[i];     /* Fetch byte from memory, XOR into CRC top byte*/
            ...
            if (checksumData.Any())
            {
                ...
                CheckSum[4] = checksumData[0]; CheckSum[5] = checksumData[1];
            }
            return CheckSum;
        }
    }
