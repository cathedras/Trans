using System.Collections.Generic;
using System.Linq;
using System.Text.RegularExpressions;
using ScintillaNET;

namespace TspUtil
{
    internal class CustomLexer
    {
        public const int StyleDefault = 0;
        public const int StyleKeyword = 1;
        public const int StyleIdentifier = 2;
        public const int StyleNumber = 3;
        public const int StyleString = 4;
        public const int StyleComment = 5;

        private const int StateUnknown = 0;
        private const int StateIdentifier = 1;
        private const int StateNumber = 2;
        private const int StateString = 3;
        private const int StateComment = 4;

        private HashSet<string> keywords;

        public void Style(Scintilla scintilla, int startPos, int endPos)
        {
            // Back up to the line start
            var line = scintilla.LineFromPosition(startPos);
            startPos = scintilla.Lines[line].Position;

            var length = 0;
            var state = StateUnknown;
            var isFinishComent = false;
            // Start styling
            scintilla.StartStyling(startPos);
            while (startPos < endPos)
            {
                var c = (char)scintilla.GetCharAt(startPos);

                REPROCESS:
                switch (state)
                {
                    case StateUnknown:
                        if (c == '"')//指示字符串开始
                        {
                            //isFinishComent = false;
                            // Start of "string"
                            scintilla.SetStyling(1, StyleString);
                            state = StateString;
                        }
                        else if (char.IsDigit(c))//指示数字
                        {
                            isFinishComent = false;
                            state = StateNumber;
                           // App.Locator.Main.AddLogMsg($"num:{startPos},{endPos},{(char)scintilla.GetCharAt(startPos)}");
                            goto REPROCESS;
                        }
                        else if (char.IsLetter(c))//指示字母
                        {
                            isFinishComent = false;
                            state = StateIdentifier;
                          //  App.Locator.Main.AddLogMsg($"ident:{startPos},{endPos},{(char)scintilla.GetCharAt(startPos)}");
                            goto REPROCESS;
                        }
                        else if (c == '#')//指示注释
                        {
                            state = StateComment;
                            goto REPROCESS;
                        }
                        else
                        {
                            if (!isFinishComent)
                            {
                                scintilla.SetStyling(1, StyleDefault);
                                //App.Locator.Main.AddLogMsg($"default:{startPos},{endPos},{(char)scintilla.GetCharAt(startPos)}");
                            }
                        }
                        break;

                    case StateString:
                        if (c == '"')
                        {
                            length++;
                            scintilla.SetStyling(length, StyleString);
                            length = 0;
                            state = StateUnknown;
                        }
                        else
                        {
                            length++;
                        }
                        break;

                    case StateNumber:
                        if (char.IsDigit(c) || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F') || c == 'x')
                        {
                            length++;
                        }
                        else
                        {
                            scintilla.SetStyling(length, StyleNumber);
                            length = 0;
                            state = StateUnknown;
                            goto REPROCESS;
                        }
                        break;

                    case StateIdentifier:
                        if (char.IsLetterOrDigit(c))
                        {
                            length++;
                        }
                        else
                        {
                            var style = StyleIdentifier;
                            var identifier = scintilla.GetTextRange(startPos - length, length);
                            if (keywords.Contains(identifier))
                                style = StyleKeyword;

                            scintilla.SetStyling(length, style);
                            length = 0;
                            state = StateUnknown;
                            goto REPROCESS;
                        }
                        break;

                    case StateComment:
                        if (c == '#')
                        {
                            length++;
                            scintilla.SetStyling(1, StyleComment);
                        }
                        else if (c == '\r')
                        {
                            var identifier = scintilla.GetTextRange(startPos-length, length);
                            //  App.Locator.Main.AddLogMsg(identifier+$"{identifier.Length},{commentLen}");
                            scintilla.SetStyling(length, StyleComment);
                            length = 0;
                            state = StateUnknown;
                            isFinishComent = true;
                        }
                        else
                        {
                            length++;
                            //startPos++;
                        }
                        break;
                }

                startPos++;
            }
        }

        public CustomLexer(string keywords)
        {
            // Put keywords in a HashSet
            var list = Regex.Split(keywords ?? string.Empty, @"\s+").Where(l => !string.IsNullOrEmpty(l));
            this.keywords = new HashSet<string>(list);
        }
    }
}
