...
namespace System.Collections.Concurrent
{
        ...
        where TKey : IComparable<TKey>
    {
        ...
        public void UpdateKey(int index, TKey keyValue)
        {
            lock (_syncLock)
            {
                if (index<Count)
                {
                    var res = new KeyValuePair<TKey, TValue>(keyValue, _minHeap.Items[index].Value);
                    _minHeap.Insert(res);
                    _minHeap.RemoveAt(index);
                }
            }
        }
        ...
        private sealed class MinBinaryHeap
        {
            ...
            public KeyValuePair<TKey, TValue> Remove()
            {
                ...
                #region add by qgc 0709
                int minIndex = 0;
                for (int i = 1; i < _items.Count;i++ )
                {
                    var entry1 = _items[minIndex];
                    var entry2 = _items[i];
